...
export const getScaleNotes = (root: string, scaleType: 'Major' | 'Minor' = 'Major'): string[] => {
    // Find the starting index in the chromatic scale (ignoring octaves for calculation)
    // We normalize to simple names without octaves for key checking
    const chromatic = BASE_NOTES.map(n => n.name); 
    // Handle enharmonics - extremely simplified for this UI visualization
    // If root is 'Db', we map it to 'C#' for index finding if Db isn't in BASE_NOTES names
    let rootIndex = chromatic.findIndex(n => n === root);
    if (rootIndex === -1) {
       // Try finding by flat/sharp alias
    ...
    const scaleNotes = intervals.map(interval => {
        ...
        // Return both sharp and flat forms to be safe for matching
        return [note.name, note.flat, note.sharp].filter(Boolean) as string[];
    }).flat();
